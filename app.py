...
def get_sensor_data():
    ...
    cnx, cursor = createConnection('sql3664795', 'sql3664795', 'aYTDzXpAwk', 'sql3.freemysqlhosting.net', '3306')
    ...
    plt.plot(x, y2, label='Temperatura')
    plt.plot(x, y3, label='Humedad')
    plt.xlabel('X')
    plt.ylabel('Y')
    plt.legend()
...
def receive_sensor_data():
    if request.headers['Content-Type'] == 'application/json':
        ...
        gas = str(data.get('gas')) # "2021-08-08 12:00:00"

        cnx, cursor = createConnection('sql3664795', 'sql3664795', 'aYTDzXpAwk', 'sql3.freemysqlhosting.net', '3306')

        add_data = ("INSERT INTO dht_sensor_data (humidity, temperature, gas) VALUES ("+humidity+","+temperature+",'"+gas+"')")
